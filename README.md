...
frontend/
  ...
  screen_wiring.test.ts            # index.htmlをテキストとして読み、画面
                                    # 遷移CSSの対応漏れを機械的に検出する
                                    # （実行はしない。既知の限界は本文参照）
  list_layout.test.ts              # index.htmlをテキストとして読み、縦
                                    # スクロール一覧のflex-shrink:0の指定漏れ
                                    # を機械的に検出する（実行はしない）
...
### 縦スクロールする一覧を `flex-direction:column` で組むときの注意点

`.log`（`#room`/`#history`/`#detail` 共通、`display:flex;flex-direction:column`）
のように、動的に行数が増える縦スクロール一覧を flex column で組んでいる場合、
**子要素に `flex-shrink:0` を付けない限り、`min-height`/`min-width` は
「下限」ではなく「行数が多いときの実質的な上限」として働きうる。**

実際に一度これで壊れた: 履歴一覧の行(`.pin`)を題名+メタ情報の2段構成に
した際、元は1行時代のタップ領域確保のためだけに付けていた
`min-height:var(--tap)`（48px、自然な高さの約68pxより小さい）が、
行数が多く `.log` の表示領域を超えたときに flexbox のデフォルト
（`flex-shrink:1`）によって**全行が48pxまで圧縮され**、2段目の
メタ情報がカードの枠からはみ出すレイアウト崩れになった。行数が少なく
オーバーフローしないときは再現しないため、実機で複数件保存してから
初めて気づいた。

Playwrightで実測して確認した内容:

- 単独表示、または行数が少なく `.log` がオーバーフローしないときは、
  各行は自然な高さ（約68px）でレンダリングされ、症状は再現しない
- 20行表示（`.log` の表示領域を超える）にすると、`flex-shrink:0` が
  無い状態では**全20行が例外なく48px（`min-height`の値そのもの）に
  圧縮され**、メタ情報が枠の外にはみ出した
- 同じ `.log` の中でも `.msg`/`.bub`（会話の吹き出し）は `min-height` を
  一切指定していないため、この罠には該当しない。24件の吹き出しを
  オーバーフローする状態で表示しても、テキストが `.bub` の外にはみ出す
  行は0件だった（flexboxの「コンテンツに基づく自動最小サイズ」の保護が
  効いたまま）。**同じ症状が過去にも起きていたのでは、という懸念は
  実測の結果、否定された**（過去の「文章が途中で切れる」症状は、
  当時対処した幅とMarkdownの問題で説明がつく）。

対処: `.pin` に `flex-shrink:0` を追加し、常に内容に基づく自然な高さを
維持するようにした（`.log` の `overflow-y:auto` にスクロールを任せる）。
`.pin` に付けていた `min-height:var(--tap)` 自体は、他の操作可能要素と
同じ「タップ領域48pxの明示」という約束事との一貫性を優先して残した
（`flex-shrink:0` により、この値が縮小後の上限として働く経路自体は
無くなっている）。

**このバグはCSSの実際のレイアウト計算に依存するため、自動テストでの
検出には限界がある。**

- `.pin` に `flex-shrink:0` が指定されているかどうかは、
  `frontend/list_layout.test.ts` が `index.html` のテキストに対する
  静的チェックとして検出できる（実際に `flex-shrink:0` を削る負の対照実験で
  確認済み）
- 一方、`min-height` の**絶対値**が実際のコンテンツの自然な高さを
  上回っていないか（＝将来また同じ罠を踏んでいないか）は、ブラウザでの
  レイアウト計算そのものが必要なため、テキスト照合はもちろん
  `deno test` のどのユニットテストでも検出できない。実際に `min-height` を
  200px（自然な高さを大幅に超える値）に変更する負の対照実験を行ったが、
  既存196本を含めどのテストも失敗しなかった。**この種の回帰は、実機また
  はPlaywright等のブラウザ実行検証でしか検出できない、という限界として
  記録する。**
...
- [ ] 17.5. **履歴一覧・詳細・再開（実機のみで確認できる配線部分）**
      - 手順16の実行後、反物画面から「履歴」を開き、直前の実行が一覧に
        ...
        されることを確認する
      - **履歴が10件以上たまった状態**で一覧をスクロールし、どの行でも
        メタ情報（終了理由・日時・構成・周回数・消費回数）がカードの
        枠内に収まっていることを確認する（実際に一度、行数が少ないと
        再現しないレイアウト崩れが起きたことがある。`min-height` の
        絶対値が自然な高さを上回っていないかは自動テストで検出できない
        ため、ここが唯一の確認手段。README「既知の限界」参照）
